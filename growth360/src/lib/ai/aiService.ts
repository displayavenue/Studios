...
export async function runAiStructured<T>(options: AiCallOptions): Promise<{
  ...
  if (options.assessmentId) {
    const count = await prisma.aiGeneration.count({
      where: { assessmentId: options.assessmentId, status: { in: ["success", "failed"] } },
    });
  ...
  const prompt = await getActivePrompt(options.useCase);
  const system =
    options.systemPrompt ||
    prompt?.content ||
    defaultSystemPrompt(options.useCase);
...
- Do NOT invent numeric pricing, fees, ROI, ad spend, or competitor facts.
...
- Do not invent competitors or contact information.`,
    ...
    console.error("[ai]", options.useCase, message);
...
function defaultSystemPrompt(useCase: AiUseCase) {
  switch (useCase) {
