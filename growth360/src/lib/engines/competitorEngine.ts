...
export async function matchCompetitors(
  ...
  const industry = industrySlug
    ? await prisma.industry.findFirst({ where: { OR: [{ slug: industrySlug }, { name: { equals: profile.industry || "", mode: "insensitive" } }] } })
    : null;
  const location = locationSlug
    ? await prisma.location.findFirst({ where: { OR: [{ slug: locationSlug }, { name: { equals: profile.location || "", mode: "insensitive" } }] } })
    : null;
