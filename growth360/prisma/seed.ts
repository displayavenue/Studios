...
async function main() {
  const passwordHash = await bcrypt.hash(
    process.env.ADMIN_PASSWORD || "Growth360Admin@2026",
    12,
  );

  await prisma.admin.upsert({
    where: { email: process.env.ADMIN_EMAIL || "admin@displayavenue.com" },
    create: {
      email: process.env.ADMIN_EMAIL || "admin@displayavenue.com",
      name: "Growth360 Admin",
      passwordHash,
      role: "admin",
    },
  ...
  // Factual competitor seed records (illustrative market peers — scores are internal ratings, not invented financial claims)
  const competitors = [
  ...
  for (const [key, name, content] of prompts) {
    const existing = await prisma.aiPromptVersion.findFirst({ where: { key }, orderBy: { version: "desc" } });
    if (!existing) {
  ...
  console.log("Growth360 seed complete");
}
